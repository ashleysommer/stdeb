...
def parse_vals(cfg,section,option, raw=False):
    """parse comma separated values in debian control file style from .cfg"""
    try:
        vals = cfg.get(section,option,raw=raw)
...
def parse_val(cfg,section,option, raw=False):
    """extract a single value from .cfg"""
    vals = parse_vals(cfg,section,option,raw=raw)
    if len(vals)==0:
        return ''
    else:
...
class DebianInfo:
    ...
    def __init__(self,
        ...
        description = parse_val(cfg, module_name, 'Description', True)
        long_description = parse_val(cfg, module_name, 'Long-Description', True)
